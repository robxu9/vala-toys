...
public class Vsc.CompletionVisitor : CodeVisitor {
	...
	{
		if (_results.classes_contain (cl.name) || !this.test_symbol (this._options, cl)) {
			return; //already visited or not interesting
		}
	...
	private bool test_symbol (SymbolCompletionFilterOptions options, Symbol symbol)
	{
		...
		if (options.error_domains) {
			...
			// if I want constructor or I don't want constructor and symbol is not a constructor
			if (options.constructors || (!options.constructors
			    && !(symbol is CreationMethod || symbol is Constructor))) {
				is_static = symbol_is_static (symbol);
				...
				if (options.constructors) {
					...
					} else if (symbol is Method && !(symbol is CreationMethod || symbol is Constructor)) {
						res = false | options.local_variables;
					} else {
	...
	{
		return (symbol is Class) || (symbol is Namespace) || (symbol is CreationMethod);
	}
	
	private bool symbol_is_static (Symbol symbol, bool instanziable_types = false)
	{
		bool res;
		if (symbol is CreationMethod || symbol is Constructor || symbol is Interface) {
			res = false;
			...
			res = true;
		} else if (symbol is Class && _options.static_symbols) {
			//class is fully visited only if looking for
			//static symbols
			var cl = (Class) symbol;
			res = class_has_static_symbols (cl);
		} else {
			res = false;
		}
		return res;
	...
	private bool class_has_static_symbols (Class cl)		
	{
